...
import win32com.client #python에서 activX 함수 호출
import pandas as pd
...
try:
    setting.drop_table_stock_code(conn)
    setting.create_table_stock_code(conn)
except:
    setting.create_table_stock_code(conn)
# AutoConnect.connectCreon()
myToken = "xoxb-2143616568480-2119135848069-KcB9662VZp2pTJCW1CmUeR8g"
...
# 크레온 플러스 공통 OBJECT
cpStatus = win32com.client.Dispatch('CpUtil.CpCybos') # CpCybos(대신증권 HTS(주식거래)프로그램) 연결 상태 확인
cpCodeMgr = win32com.client.Dispatch('CpUtil.CpStockCode') # CpStockCode 연결 상태 확인, CYBOS에서 사용되는 주식코드 조회 작업을 함.
cpTradeUtil = win32com.client.Dispatch('CpTrade.CpTdUtil') # 거래관련 유틸
cpStock = win32com.client.Dispatch('DsCbo1.StockMst') # 주식 종목 현재가
cpOhlc = win32com.client.Dispatch('CpSysDib.StockChart') # [주식, 업종, ELW 차트]
cpBalance = win32com.client.Dispatch('CpTrade.CpTd6033')# [계좌별 잔고 평가현황]
cpCash = win32com.client.Dispatch('CpTrade.CpTdNew5331A') #[계좌별 매수 가능금액/수량]
cpOrder = win32com.client.Dispatch('CpTrade.CpTd0311')  # [장내주식/코스닥주식/ELW 현금주문]
...
def get_ohlc(code, qty):
    ...
    cpOhlc.SetInputValue(6, ord('T'))        # T:틱 단위
    cpOhlc.SetInputValue(9, ord('1'))        # 0:무수정주가, 1:수정주가(주식수의 변화에 따른 가치를 가짐)
    cpOhlc.BlockRequest() #데이터 요청
    count = cpOhlc.GetHeaderValue(3)   # 3:수신개수
    columns = ['open', 'high', 'low', 'close']
    index = []
    rows = []
    for i in range(count): 
        index.append(cpOhlc.GetDataValue(0, i)) #종목 코드
        rows.append([cpOhlc.GetDataValue(1, i), cpOhlc.GetDataValue(2, i), #필드 개수, 필드명의 배열
            cpOhlc.GetDataValue(3, i), cpOhlc.GetDataValue(4, i)]) #수신 개수, 마지막 봉 틱수
...
def get_stock_balance(code):
    ...
    stocks = []
    for i in range(cpBalance.GetHeaderValue(7)): # 수신개수
    ...
    if code == 'ALL':
        ...
        stock_name = cpCodeMgr.CodeToName(code) # code에해당하는종목명을반환합니다
        return stock_name, 0
...
def get_target_price(code):
    """매수 목표가를 반환한다."""
    """인자로 받은 종목의 현재가를 반환한다."""
    for i in range(len(code)):
        cpStock.SetInputValue(0, code[i])  # 종목코드에 대한 가격 정보
        cpStock.BlockRequest()
        price = cpStock.GetHeaderValue(11)   # 현재가
        print(price)
        setting.insert_table_stock_code(conn,code[i],price)
    from create_modeldata import check_model
    from predict_data_to_mysql import Predict
    check_model()
    Predict()
    return print('clear_get_target')

    # try:
    #     time_now = datetime.now()
    #     str_today = time_now.strftime('%Y%m%d')
    #     #[index, 'open', 'high', 'low', 'close']
    #     #[종목코드(code), 필드 개수, 필드명, 수신 개수, 마지막 봉 틱 수]
    ...
    #     return None
    #-------------------------------------------------------------------------------
def get_target_price_mysql(code):
    table_stock_code = setting.fetch_stock_code_where_code(conn,code)
    table_stock_code= pd.DataFrame(table_stock_code)
    print(table_stock_code)
    return table_stock_code
...
def get_movingaverage(code, window):
    ...
    try:
        ...
        ma = closes.rolling(window=window).mean() #window개수만큼 이동평균의 평균
...
def buy_etf(code):
    ...
    try:
        ...
        if code in bought_list: # 매수 완료 종목이면 더 이상 안 사도록 함수 종료
            ...
            return False
        time_now = datetime.now()
        current_price, ask_price, bid_price = get_current_price(code) #현재가, 매수호가, 매도호가
        target = setting.fetch_all_predict(conn)
        ###########################################
        target_price = get_target_price_mysql(code)
        # target_price = target.values[0][0]    # 매수 목표가
        ###########################################
        ma5_price = get_movingaverage(code, 5)   # 5일 이동평균가
        ma10_price = get_movingaverage(code, 10) # 10일 이동평균가
        buy_qty = 0        # 매수할 수량 초기화
        if ask_price > 0:  # 매수호가가 존재하면
            buy_amount = total_cash * buy_amount_code(code)
...
def buy_amount_code(code):
    ...
    for counter in range(len(data)):
        if sort_data['data'][counter][0] == code:
            result = sort_data['data'][counter][1]
    return result
...
def target_list(n):
    ...
    for counter in range(len(data)):
        setting.update_stock_code(conn,sort_data['code'][counter],float(sort_data['invest_pct'][counter]))

if __name__ == '__main__':
    symbol_list = ['A005930','A004370','A035720','A068270','A066570','A013520']#삼성전자, 농심 ,카카오, 셀트리온,LG전자, 화승코퍼레이션
    ...
    print('점검 패스')
    get_target_price(symbol_list)
    target_list(target_buy_count)
    ...
    # printlog('종목별 주문 비율 :')
    # print(data[['code','invest_pct']])
    # printlog('종목별 주문 금액 :')
    # print(data[['invest_pct']]*total_cash)
